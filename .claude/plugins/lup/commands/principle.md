...
1. **Guidance** (`src/inkwell/devtools/harness/content/guidance.py`, plus
...
2. **Template guidance** (`src/inkwell/devtools/harness/content/template_sections.py`)
...
3. **All skill modules** (`src/inkwell/devtools/harness/content/skills/*.py`,
...
4. **Canonical policy** (the library's `lup.policy` plus the `HookSet` in
   `src/inkwell/devtools/harness/catalog.py`; everything under .claude/plugins/lup/hooks/ under Claude Code, .codex/plugins/lup/hooks/ under Codex is generated from these — never edit it directly)
...
5. **Agent code** (`src/inkwell/agent/`)
...
7. **Environment code** (`src/inkwell/environment/`)
...
**Group 5: Devtools & automation** (`lup.devtools` and `src/inkwell/devtools/`)
