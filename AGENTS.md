...
- **The part is written, not anchored.** Routed as the piece a run *replaces*, the standing text becomes the plan and its section order survives the rewrite. Routed as material, the run answers what the part has to establish — and what the standing text carried is kept by the inheritance pass, whose audit is subtracted from what the two texts hold rather than believed.
- **The plan comes from outside the run**, because nothing holding one subsection can say what that subsection should open with or how much of its chapter it is owed. A brief is composed from what only the work knows and the plan stage records it; recorded rather than skipped, since skipping saves the same call and silently takes the Plan tab and the plan on disk with it.
...
`distil` is the fourth, and runs lazily rather than in that order: a document is read whole the first time a filter reaches it, and what it establishes is cached under its `content_sha256` for good. The judging cannot carry it — the judge reads six pages on purpose, and a finding is on page 34. `manuscript research` puts the two together: it distils what a work's filter reaches, places findings on the parts they bear on by reading the tree's titles, and turns each new placement into a `finding` change fact, so the ordinary sweep dirties those parts and the loop names the paper. Placing moves nothing until somebody says so; the reading is priced first with `--dry-run`.
...
Report at the level asked. A finding is what an investigation establishes; its steps are not the finding. Reconstructing the whole progression buries the answer.
