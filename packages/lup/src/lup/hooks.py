...
def create_large_read_hook() -> HooksConfig:
    ...
    async def block_tool_results_read(
        ...
        if TOOL_RESULTS_SEGMENT not in file_path:
            return SyncHookJSONOutput()

        return deny_hook_output(
            "Reading persisted-output temp files causes infinite loops. "
            "Re-read the original file with offset and limit parameters "
            "to read it in chunks, or use the structured artifacts "
