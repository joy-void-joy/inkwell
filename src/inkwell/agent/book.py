...
chapter's file. The outline is one more single-writer file: the book stage lays
it out and a chapter run only ever reads it. Each write is an atomic
temp-and-rename through ``publish_atomic``, so a reader assembling the book sees
whole chapter records or none, never a half-written one. Two runs of the *same*
chapter do land on one path; there the rename decides, and the later record
replaces the earlier one whole rather than blending with it.
...
def records[Held: BaseModel](
    directory: Path, kind: type[Held], skip: Path | None = None
) -> Iterator[Held]:
    ...
    One sibling path may be skipped, for the book directory that holds the
    outline beside its chapter records: the outline is a record of a different
    kind, and reading it as this one would report a fault where there is none.
    """
...
class BookStore(BaseModel, frozen=True):
    ...
    def citations_path(self, placement: ChapterPlacement) -> Path:
        ...
        return self.citations_dir(placement.book) / self.chapter_file(placement)
    ...
    def load(self, book: str) -> BookRecord:
        ...
        return BookRecord(
            ...
            chapters=sorted(
                records(
                    self.book_dir(empty.book),
                    ChapterRecord,
                    skip=self.outline_path(empty.book),
                ),
