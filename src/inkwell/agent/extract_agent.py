...
def assemble_sources(manifest: ExtractionManifest) -> tuple[list[str], list[str]]:
    ...
    ``--- Source: ... ---`` sections ready to join into the conversation, and
    ``unrecovered`` holds the raw inputs whose content could not be read and
    that the caller should deterministically re-extract.
    """
    blocks: list[str] = []
    ...
    for entry in manifest.sources:
        ...
        if text.strip():
            blocks.append(f"--- Source: {entry.raw_input} ---\n\n{text}")
        else:
