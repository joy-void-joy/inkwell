...
def overwrite_env_file(path: Path, values: dict[str, str]) -> None:
...
def clear_env_file(path: Path, keys: Iterable[str]) -> None:
    """Drop specific keys from an env file. Idempotent — a no-op if none present."""
    existing = read_env_file(path)
    drop = set(keys)
    if not existing.keys() & drop:
        return
    overwrite_env_file(path, {k: v for k, v in existing.items() if k not in drop})
...
RESETTABLE_ENV_KEYS: dict[str, tuple[str, ...]] = {
    ...
    "exa": ("EXA_API_KEY",),
    "fred": ("FRED_API_KEY",),
    "claude-login": ("CLAUDE_CONFIG_DIR",),
}
...
def reset_integration(integration: str, profile: str | None = None) -> None:
    ...
    match integration:
        case "google":
            creds, token = google_paths_for_profile(profile)
