...
class LengthBudget(BaseModel):
    ...
    def render(self) -> str:
        ...
        return (
            ...
            f"{placed} Plan it to land between {self.floor():,} and "
            f"{self.ceiling():,} words. Nothing rejects a finished draft for "
            f"its length, but this is the room the work gives this part, and a "
            f"plan that does not fit it obliges a writer to overrun rather than "
            f"buying the piece any more room. Where the material needs more, "
            f"plan what fits and ask: the allocation is the author's to settle "
            f"and costs a note telling them what the extra words buy. That the "
            f"subject is large is not such a reason, "
